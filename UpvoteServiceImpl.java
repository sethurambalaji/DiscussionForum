...
public class UpvoteServiceImpl implements UpvoteService {
    ...
    private Upvote getUpvote(User user, Question question, Reply reply) {
        for (Upvote upvote : upvotes) {
			...
			*/
            if (reply == null && upvote.getQuestion() != null) {
                if (question.getId() == upvote.getQuestion().getId()) {
                    if (user.getId() == upvote.getUser().getId()) {
                        return upvote;
                    }
                }
            }
            if (question == null && upvote.getReply() != null) {
                if (reply.getId() == upvote.getReply().getId()) {
                    if (user.getId() == upvote.getUser().getId()) {
                        return upvote;
                    }
                }
